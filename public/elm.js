...
var $author$project$Main$UpdateWindowWidth = function (a) {
	return {$: 'UpdateWindowWidth', a: a};
};
...
var $author$project$Main$fWeightedAverage = F4(
	function (fa, fb, _v0, x) {
		var xl = _v0.a;
		var xr = _v0.b;
		if (_Utils_cmp(x, xl) < 1) {
			return fa(x);
		} else {
			if (_Utils_cmp(x, xr) > -1) {
				return fb(x);
			} else {
				var k = ($elm$core$Basics$cos(((xr - x) / (xr - xl)) * $elm$core$Basics$pi) + 1) / 2;
				return (fa(x) * (1 - k)) + (fb(x) * k);
			}
		}
	});
var $author$project$Main$calculatePoints = F3(
	function (expression, smooth, maxValue) {
		var smoothRange = 1.5;
...
var $author$project$Main$init = F3(
	function (flags, url, key) {
		...
		var initialPoints = A3($author$project$Main$calculatePoints, initialExpression, true, 10);
		var state = {
			...
			input: 'sin x * 20',
			lastFrameTime: 0,
			parsedExpression: $elm$core$Maybe$Just(initialExpression),
			requestFrame: true,
			smoothEnds: true,
			spriteHead: $author$project$Main$Loading,
			spriteTail: $author$project$Main$Loading,
			windowWidth: 200
		};
		return _Utils_Tuple2(
			...
			A2(
				$elm$core$Task$perform,
				function (viewport) {
					return $author$project$Main$UpdateWindowWidth(
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				F2(
					function (x, y) {
						return $author$project$Main$UpdateWindowWidth(x);
					})),
				$elm$browser$Browser$Events$onAnimationFrame($author$project$Main$AnimationFrame)
			]));
};
...
var $author$project$Expression$mergeFromLeft = F3(
	function (f, i, xs) {
		mergeFromLeft:
		while (true) {
			if (!xs.b) {
				...
				i = $temp$i;
				xs = $temp$xs;
				continue mergeFromLeft;
			}
		}
	});
var $author$project$Expression$mergeFromRight = F3(
	function (f, i, xs) {
		if (!xs.b) {
			return i;
		} else {
			...
			return A3(
				...
				i,
				a,
				A3($author$project$Expression$mergeFromRight, f, b, xr));
		}
	});
...
var $author$project$Expression$parseSequence = F2(
	function (sep, component) {
						...
						A2(
							...
							A2(
								...
								$elm$parser$Parser$succeed(
									F2(
										function (sep1, component1) {
											return $elm$parser$Parser$Loop(
												A2(
													$elm$core$List$cons,
													_Utils_Tuple2(sep1, component1),
													revParts));
										})),
								$elm$parser$Parser$spaces),
							A2($elm$parser$Parser$ignorer, sep, $elm$parser$Parser$spaces)),
						component),
		...
		return A2(
			...
			A2(
				...
				A2(
					...
					$elm$parser$Parser$spaces),
				A2($elm$parser$Parser$ignorer, component, $elm$parser$Parser$spaces)),
			A2(
				$elm$parser$Parser$ignorer,
	...
	function (foldDirection, seps, component) {
		...
		var mergeFunction = function () {
			if (foldDirection.$ === 'Left') {
				return $author$project$Expression$mergeFromLeft;
			} else {
				return $author$project$Expression$mergeFromRight;
			}
		}();
		return A2(
			$elm$parser$Parser$map,
			...
			A2($author$project$Expression$parseSequence, sepsParser, component));
	});
...
var $author$project$Main$updateComputedValues = function (model) {
	var maxValue = model.extraLong ? 50 : 10;
		...
		model,
		{
			computedValues: A2(
				...
				A2(
					$elm$core$Maybe$map,
					function (e) {
						return A3($author$project$Main$calculatePoints, e, model.smoothEnds, maxValue);
					},
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'TextureLoadedTail':
				if (msg.a.$ === 'Nothing') {
					var _v2 = msg.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{spriteTail: $author$project$Main$Failure}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								spriteTail: $author$project$Main$Success(texture)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'TextureLoadedHead':
				if (msg.a.$ === 'Nothing') {
					var _v3 = msg.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{spriteHead: $author$project$Main$Failure}),
						$elm$core$Platform$Cmd$none);
				} else {
					var texture = msg.a.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								spriteHead: $author$project$Main$Success(texture)
							}),
						$elm$core$Platform$Cmd$none);
				}
			...
			case 'InputChanged':
				...
				var newModel = function () {
					...
					if (_v4.$ === 'Err') {
						...
						return $author$project$Main$updateComputedValues(
							_Utils_update(
								model,
								{
									expressionError: $elm$core$Maybe$Nothing,
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								...
								lastFrameTime: $elm$time$Time$posixToMillis(time),
								requestFrame: !_Utils_eq(newPoints, model.computedValues)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'SetSmoothing':
				var smooth = msg.a;
				return _Utils_Tuple2(
					$author$project$Main$updateComputedValues(
						_Utils_update(
							model,
							{smoothEnds: smooth})),
					$elm$core$Platform$Cmd$none);
...
var $author$project$Main$blue = A3($rtfeldman$elm_css$Css$rgb, 66, 188, 245);
...
var $author$project$Main$loadMap2 = F3(
	function (f, la, lb) {
		var _v0 = _Utils_Tuple2(la, lb);
		_v0$1:
		while (true) {
			_v0$3:
			while (true) {
...
var $author$project$Main$path1 = function (points) {
	if (!points.b) {
		return A2(
		...
		return A2(
			$joakin$elm_canvas$Canvas$path,
			x,
			A2($elm$core$List$map, $joakin$elm_canvas$Canvas$lineTo, xr));
	}
};
var $author$project$Main$thiccLine = F6(
	function (y, l, points, o1, o2, color) {
		var pointsOnCanvas = _Utils_ap(
		...
		return A2(
			...
			_List_fromArray(
				[
					$author$project$Main$path1(pointsOnCanvas)
				]));
	});
...
var $author$project$Main$viewSpriteAt = F2(
	function (_v0, sprite) {
		var x = _v0.a;
		var y = _v0.b;
		return A3(
...
var $author$project$Main$viewCanvas = F4(
	function (points, spriteHead, spriteTail, w) {
		var y = 15;
		var o = 0;
		var x0 = function (x) {
			return ((x - 133) + (w / 2)) - o;
			...
			A2(
				$elm$core$Maybe$withDefault,
				0,
				A2($elm$core$Array$get, 0, points)));
		var xl = function (x) {
			return ((x - 133) + (w / 2)) - o;
		}(
			A2(
				...
				A2(
					$elm$core$Array$get,
					$elm$core$Array$length(points) - 1,
					points)));
		var h = 520 + ($elm$core$Array$length(points) * 2);
		return A3(
			...
			_List_fromArray(
				[
					...
					A6(
					$author$project$Main$thiccLine,
					y,
					(w / 2) - o,
					points,
					0,
					71,
					A3($avh4$elm_color$Color$rgb255, 223, 188, 152)),
					A6(
					$author$project$Main$thiccLine,
					y,
					(w / 2) - o,
					...
					0,
					12,
					A3($avh4$elm_color$Color$rgb255, 194, 141, 102)),
					A6(
					$author$project$Main$thiccLine,
					y,
					(w / 2) - o,
					...
					58,
					71,
					A3($avh4$elm_color$Color$rgb255, 181, 122, 94)),
					A6($author$project$Main$thiccLine, y, (w / 2) - o, points, 0, 1, $avh4$elm_color$Color$black),
					A6($author$project$Main$thiccLine, y, (w / 2) - o, points, 70, 71, $avh4$elm_color$Color$black),
					A2(
					...
					_Utils_Tuple2(
						xl,
						(y + 254) + ($elm$core$Array$length(points) * 2)),
					spriteTail)
				]));
	});
...
var $author$project$Main$slateGrey = A3($rtfeldman$elm_css$Css$rgb, 47, 49, 54);
...
var $author$project$Main$darkSection = $rtfeldman$elm_css$Css$batch(
	_List_fromArray(
		[
			$rtfeldman$elm_css$Css$backgroundColor($author$project$Main$slateGrey),
			$rtfeldman$elm_css$Css$color($author$project$Main$white),
...
var $author$project$Main$viewEquationComponent = F2(
	function (expressionError, input) {
		var contents = A4(
			...
			_List_fromArray(
				[
					...
					function () {
					if (expressionError.$ === 'Nothing') {
						return $rtfeldman$elm_css$Css$borderColor($author$project$Main$white);
					} else {
						var error = expressionError.a;
						return $rtfeldman$elm_css$Css$borderColor(
							A3($rtfeldman$elm_css$Css$rgb, 245, 66, 120));
					}
				}(),
		...
		return A4(
			...
			_List_fromArray(
				[
					$author$project$Main$darkSection,
...
var $author$project$Main$viewHeader = function () {
	var links = _List_fromArray(
		[
			_Utils_Tuple2('Made by Paf', 'https://twitter.com/ProbsAFox'),
			_Utils_Tuple2('Character is Kita', 'https://twitter.com/kitacatter'),
			_Utils_Tuple2('Art by Cianiati', 'https://twitter.com/ScribbleServal'),
			_Utils_Tuple2('Source', 'https://github.com/DXsmiley/longboi')
		]);
	var linksHtml = A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_Nil,
...
var $author$project$Main$viewHelpFooter = function () {
	var code = function (t) {
		return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text('Values of '),
								code('x'),
								$rtfeldman$elm_css$Html$Styled$text(' range from '),
								code('0'),
								...
								$rtfeldman$elm_css$Html$Styled$text(' to '),
								code('50'),
								$rtfeldman$elm_css$Html$Styled$text(' in extra long mode.')
							]))
					]))
			]));
}();
...
var $author$project$Main$styleLoadingScreen = $rtfeldman$elm_css$Css$batch(
	_List_fromArray(
		[
...
var $author$project$Main$viewLoadFailure = A4(
	$rtfeldman$elm_css$Html$Styled$styled,
	$rtfeldman$elm_css$Html$Styled$div,
	_List_fromArray(
		[$author$project$Main$styleLoadingScreen]),
	_List_Nil,
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					$rtfeldman$elm_css$Html$Styled$text('Loading resources failed. Try refreshing the page.')
				]))
		]));
var $author$project$Main$TextureLoadedHead = function (a) {
	return {$: 'TextureLoadedHead', a: a};
};
var $author$project$Main$TextureLoadedTail = function (a) {
	return {$: 'TextureLoadedTail', a: a};
};
...
var $author$project$Main$viewLoadingScreen = A4(
	$rtfeldman$elm_css$Html$Styled$styled,
	$rtfeldman$elm_css$Html$Styled$div,
	_List_fromArray(
		[$author$project$Main$styleLoadingScreen]),
			...
			A3(
				$joakin$elm_canvas$Canvas$toHtmlWith,
				{
					...
					textures: _List_fromArray(
						[
							A2($joakin$elm_canvas$Canvas$Texture$loadFromImageUrl, './public/kita-head.png', $author$project$Main$TextureLoadedHead),
							A2($joakin$elm_canvas$Canvas$Texture$loadFromImageUrl, './public/kita-tail.png', $author$project$Main$TextureLoadedTail)
						]),
					width: 0
				},
				_List_Nil,
...
var $author$project$Main$SetExtraLong = function (a) {
	return {$: 'SetExtraLong', a: a};
};
var $author$project$Main$SetSmoothing = function (a) {
	return {$: 'SetSmoothing', a: a};
};
...
var $author$project$Main$viewToggles = F2(
	function (smoothEnds, extraLong) {
		var toggle = F3(
			function (text_, isOn, command) {
				return A4(
					$rtfeldman$elm_css$Html$Styled$styled,
					$rtfeldman$elm_css$Html$Styled$div,
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									isOn ? $rtfeldman$elm_css$Css$backgroundColor($author$project$Main$blue) : $rtfeldman$elm_css$Css$backgroundColor($author$project$Main$white),
									$rtfeldman$elm_css$Css$flexShrink(
									$rtfeldman$elm_css$Css$num(0)),
									$rtfeldman$elm_css$Css$flexGrow(
									$rtfeldman$elm_css$Css$num(0)),
									...
									$rtfeldman$elm_css$Css$borderStyle($rtfeldman$elm_css$Css$solid),
									$rtfeldman$elm_css$Css$borderColor($author$project$Main$white),
									$rtfeldman$elm_css$Css$display($rtfeldman$elm_css$Css$inlineBlock)
								]),
							_List_fromArray(
								[
		...
		return A4(
			...
			_List_fromArray(
				[
					A3(toggle, 'Smooth Ends', smoothEnds, $author$project$Main$SetSmoothing),
					A3(toggle, 'Extra Long', extraLong, $author$project$Main$SetExtraLong)
				]));
	});
var $rtfeldman$elm_css$Css$visited = $rtfeldman$elm_css$Css$pseudoClass('visited');
var $author$project$Main$view = function (model) {
	var loadState = A3(
		$author$project$Main$loadMap2,
		F2(
			function (a, b) {
				return _Utils_Tuple2(a, b);
			}),
	...
	var body = function () {
		switch (loadState.$) {
			case 'Failure':
				return $author$project$Main$viewLoadFailure;
			case 'Loading':
				return $author$project$Main$viewLoadingScreen;
			default:
				var _v1 = loadState.a;
				var spriteHead = _v1.a;
				var spriteTail = _v1.b;
				return A2(
					$rtfeldman$elm_css$Html$Styled$div,
					_List_Nil,
					_List_fromArray(
						[
							...
							A3($rtfeldman$elm_css$Html$Styled$Lazy$lazy2, $author$project$Main$viewToggles, model.smoothEnds, model.extraLong),
							A3($rtfeldman$elm_css$Html$Styled$Lazy$lazy2, $author$project$Main$viewEquationComponent, model.expressionError, model.input),
							A4(
							$rtfeldman$elm_css$Html$Styled$styled,
							$rtfeldman$elm_css$Html$Styled$div,
							_List_fromArray(
						...
						[
							...
							_List_fromArray(
								[
									...
									$rtfeldman$elm_css$Css$px(0)),
									$rtfeldman$elm_css$Css$backgroundColor($author$project$Main$white)
								])),
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$rtfeldman$elm_css$Css$color($author$project$Main$blue)
										]))
								]))
						]))),
